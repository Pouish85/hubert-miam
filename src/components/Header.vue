<template>
    <div class="header h-24 w-[80%] flex justify-center items-center space-x-6 relative">
            <RouterLink to="/" class="text-4xl text-left text-green-800 text-shadow-lg shadow-green-800 max-w-4/12 absolute left-4">Hubert-Miam</RouterLink>
            <div v-if="showInput" class="wrapper--input relative w-6/12 flex flex-col">
                ...
                <div class="searchResultPopup absolute w-full bg-gray-200 opacity-90 top-full border border-gray-100 rounded-lg ">
                    <RouterLink v-for="(restaurant, i) in searchResult" :key="i" :to="{name: 'Restaurant', params: {restaurantName: restaurant.restaurantName}}">
                        <div class="resultContainer flex p-2 items-center hover:bg-slate-400">
    ...
    setup(props) {
        ...
        watch(user_search, newValue => {
            ...
            if (newValue.length >= 3) {
                ...
                let search = props.allRestaurants.filter(restaurant => regex.test(restaurant.restaurantName.toLowerCase()));
